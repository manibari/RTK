...
describe("InMemoryGraphRepository", () => {
  ...
  describe("characters", () => {
    it("creates and retrieves a character", async () => {
      await repo.createCharacter({ id: "a", name: "A", traits: ["brave"], military: 2, intelligence: 0, charm: 0 });
      const char = await repo.getCharacter("a");
      expect(char).toEqual({ id: "a", name: "A", traits: ["brave"], military: 2, intelligence: 0, charm: 0 });
    });
    ...
    it("lists all characters", async () => {
      await repo.createCharacter({ id: "a", name: "A", traits: [], military: 0, intelligence: 0, charm: 0 });
      await repo.createCharacter({ id: "b", name: "B", traits: [], military: 0, intelligence: 0, charm: 0 });
  ...
  describe("graph queries", () => {
    it("builds character graph with BFS", async () => {
      await repo.createCharacter({ id: "center", name: "Center", traits: [], military: 0, intelligence: 0, charm: 0 });
      await repo.createCharacter({ id: "n1", name: "N1", traits: [], military: 0, intelligence: 0, charm: 0 });
      await repo.createCharacter({ id: "n2", name: "N2", traits: [], military: 0, intelligence: 0, charm: 0 });
    ...
    it("respects depth limit", async () => {
      await repo.createCharacter({ id: "a", name: "A", traits: [], military: 0, intelligence: 0, charm: 0 });
      await repo.createCharacter({ id: "b", name: "B", traits: [], military: 0, intelligence: 0, charm: 0 });
      await repo.createCharacter({ id: "c", name: "C", traits: [], military: 0, intelligence: 0, charm: 0 });
      await repo.setRelationship({ sourceId: "a", targetId: "b", intimacy: 50, relationshipType: "neutral" });
